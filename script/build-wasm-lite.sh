...
emcc "${sources[@]}" "${common[@]}" \
  -s EXPORT_ES6=0 -s EXPORT_NAME="OrzAudioKit" -s ENVIRONMENT='web,worker' \
  -o "$OUT/orz_audio_builtin.js"

# ESM loader exported by the npm package and usable in Node-based consumers.
emcc "${sources[@]}" "${common[@]}" \
  -s EXPORT_ES6=1 -s EXPORT_NAME="OrzAudioCoreModule" -s ENVIRONMENT='web,worker,node' \
  -o "$OUT/orz_audio_builtin.mjs"

test -s "$OUT/orz_audio_builtin.js"
test -s "$OUT/orz_audio_builtin.mjs"
